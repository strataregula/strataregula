...
def slow_measurement(func, iterations=1000):
    ...
    start_memory = sys.getsizeof(gc.get_objects())  # 超重い！全オブジェクト取得
    
    start_time = time.perf_counter()
    for _ in range(iterations):
        result = func()  # 関数呼び出しオーバーヘッド
    end_time = time.perf_counter()
    
    gc.collect()  # また重い！
    end_memory = sys.getsizeof(gc.get_objects())  # また超重い！
...
def demonstrate_measurement_overhead():
    ...
    # 遅い測定（GC付き）
    start = time.perf_counter()
    slow_result = slow_measurement(lightweight_func, 100)  # 回数減らす
    slow_measurement_time = time.perf_counter() - start
    print(f"   Slow measurement: {slow_measurement_time*1000:.4f}ms")
    
    # 高速測定
    start = time.perf_counter()
    fast_result = fast_measurement(lightweight_func, iterations)
